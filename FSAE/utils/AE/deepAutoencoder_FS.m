function [cost,grad] = deepAutoencoder_FS(theta, layersizes, data, label, P, L_b, L_w,lambda1,lambda2)
...
if nargout > 1
    ...
    Wgrad{1} = outderv * data'+lambda1*P'*P*h{i-1}*(L_w-lambda2*L_b).* dh{i-1}* data';
    bgrad{1} = sum(outderv+lambda1*P'*P*h{i-1}*(L_w-lambda2*L_b).* dh{i-1},2);
